...
local function update(capabilty, state, device, cnt)
  if (capabilty) then 
    ...
    out[cnt].capabilities[capability_cnt].type = statesMapSLS2YA[state].type
    out[cnt].capabilities[capability_cnt].state = {}
    out[cnt].capabilities[capability_cnt].state.instance = statesMapSLS2YA[state].instance
  else
    property_cnt = property_cnt + 1
    out[cnt].properties[property_cnt] = {}
    out[cnt].properties[property_cnt].type = statesMapSLS2YA[state].type
    out[cnt].properties[property_cnt].state = {}
    out[cnt].properties[property_cnt].state.instance = statesMapSLS2YA[state].instance
  end
  ...
  for dev_key, dev in pairs(query.request_data) do
    ...
    for i = 1, #dev.custom_data.states do
      ...
      if (state == "state") -- вкл/выкл 
        ...
        then 
          local value = statesMapSLS2YA[update(true, state, device, dev_key)]
      ...
      elseif (state == "led_state") then -- вкл/выкл SLS Led
        local value = fn.json_decode(update(true, state, device, dev_key))
        out[dev_key].capabilities[capability_cnt].state.value = statesMapSLS2YA[value.mode]
